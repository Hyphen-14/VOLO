...
$passengers = $_GET['passengers'] ?? 1; // <--- TANGKAP DATA PENUMPANG
...
// --- 3. HITUNG TOTAL HARGA (MULTIPLIER PENUMPANG) ---
$price_per_person = $flight_dep['price'];
if ($flight_ret) {
    $price_per_person += $flight_ret['price'];
}
...
// --- 5. LOGIKA SIMPAN KE DATABASE ---
if (isset($_POST['confirm_booking'])) {
    $user_id = $_SESSION['user_id'];
    
    // A. INSERT KE 'bookings' (Simpan jumlah penumpang juga)
    $insert_header = mysqli_query($conn, "INSERT INTO bookings (user_id, total_amount, status, total_passengers) 
                                          VALUES ('$user_id', '$grand_total', 'waiting_payment', '$passengers')");
    
    if ($insert_header) {
        $new_booking_id = mysqli_insert_id($conn); 
        
        // B. INSERT TIKET BERANGKAT
        ...
        echo "<script>alert('Booking Confirmed!'); window.location = 'my_tickets.php';</script>";
    } else {
...
<body class="center-mode">
    ...
    <div class="glass-container" style="width: 600px; text-align: left;">
        ...
        <div style="background: rgba(255,255,255,0.1); padding: 10px; border-radius: 10px; margin-bottom: 15px; text-align: center;">
            ...
            <strong style="font-size: 1.2em;"><?= $passengers; ?> Person(s)</strong>
        </div>
        ...
        <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 10px; margin-top: 20px;">
            ...
            <form method="POST" style="display: flex; gap: 10px;">
                <input type="text" name="promo_code" value="<?= $promo_code_input; ?>" class="form-input" placeholder="Enter Code" style="padding: 10px;">
        ...
        <div style="margin-top: 20px; border-top: 1px dashed #555; padding-top: 15px;">
            <div style="display: flex; justify-content: space-between; color: #ccc;">
                <span>Price per person (x<?= $passengers; ?>)</span>
